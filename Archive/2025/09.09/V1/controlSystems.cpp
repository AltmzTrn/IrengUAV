...
void controlSystems_update() {
  ...
  int16_t desroll    = mapCRSFtoPWM(rcChannelValues[0]) - 1500;
  int16_t despitch     = mapCRSFtoPWM(rcChannelValues[1]) - 1500;
  int16_t desyaw      = mapCRSFtoPWM(rcChannelValues[3]) - 1500;

  // Mix for actuators (you can tune the math later)
  to_actuator[0] = 300 + throttle;                      // Motor
  to_actuator[1] = 1500 - desroll + desyaw;            // Front servo
  to_actuator[2] = 1500 + desroll - despitch + desyaw;     // Left servo
  to_actuator[3] = 1500 + desroll + despitch + desyaw;     // Right servo
